...
Membuat button untuk setiap products
```dart
ElevatedButton(
  onPressed: () {
    Navigator.push(
...
Membuat page untuk menampilkan setiap item. Disini kita menampilkan setiap atribut dari models product.
```dart
import 'package:flutter/material.dart';
import 'package:chickendaddy_flutter/models/product_entry.dart';
...
Secara tidak langsung pada tugas-tugas sebelum kita sudah membuat endpoint /json yang memfilter berdasarkan logged in user.
Maka dari itu, dengan melakukan request terhadap 127.0.0.1:8000/json, kita sudah mengimplementasikan filter tersebut.
